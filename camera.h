...
class camera {
    public:
        ...
        double vfov = 90; // vertial fov
        point3 lookfrom = point3( 0, 0, 0); // point camera is looking from
        point3 lookat = point3(0, 0, -1); // point camera is looking at
        vec3 vup = vec3(0, 0, 1); // camera-relative "up" direction
        
        double defocus_angle = 0; // variation angle of rays through each pixel
        double focus_dist = 10; // distance from camera lookfrom point to plane perfect focus
    ...
    private:
        ...
        void initialize() {
            ...
            // calculate the vectors across the horiz and down the vert viewport edges
            vec3 viewport_u = viewport_width * u;
            vec3 viewport_v = viewport_height * -v;

            // calculate the horiz and vert delta vectors from pixel to pixel
        ...
        color ray_color(const ray&r, int depth, const hittable& world) const {
            // check if ray bounce limit exceeded
            if (depth <= 0)
                return color(0, 0, 0);
